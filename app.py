...
if 'pool_standard' not in st.session_state:
    ...
    st.session_state.pool_surprise = [
        ...
        {"שם": "סטיב ג'ובס", "תואר": "יזם", "התמחות": "חדשנות וחזון"},
        {"שם": "מרקוס אורליוס", "תואר": "קיסר רומי", "התמחות": "חוסן מנטלי"},
        {"שם": "לאונרדו דה וינצ'י", "תואר": "גאון רב-תחומי", "התמחות": "יצירתיות פותרת בעיות"},
...
if st.button("🔄 רענן והחלף משתתפים (4 מתוך 6)"):
    refresh_cabinet()

# הצגת המשתתפים בצורה מסודרת
...
if st.button("🔍 צור שאלון אבחון"):
    if idea:
        ...
        prompt = f"נושא: {idea}. קבינט: {names}. נסח 4 שאלות אבחון קצרות ופשוטות בשפה יומיומית. החזר JSON בלבד: [{{'q': 'שאלה', 'options': ['א','ב','ג']}}, ...]"
...
if 'qs' in st.session_state:
    ...
    if st.button("🚀 הפק 5 תובנות אסטרטגיות"):
        names = [m['שם'] for m in st.session_state.current_cabinet]
        prompt = f"נושא: {idea}. תשובות: {ans_list}. קבינט: {names}. כתוב 5 תובנות עמוקות ופשוטות בשפה יומיומית. לאחר מכן טבלה: בעיה, פתרון, דרך, תפוקות, תשומות."
        with st.spinner("מגבש החלטות..."):